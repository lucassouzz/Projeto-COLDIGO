	...
	COLDIGO.produto.cadastrar = function(){
		...
		if((produto.categoria=="")||(produto.marcaId=="")||(produto.modelo=="")
			...
			$.ajax({
				...
				error: function (info) {
					COLDIGO.exibirAviso("Erro ao cadastrar um novo produto: "+ infor.status + " - " + info.statusText);
				}
		...
		if(listaDeProdutos != undefined && listaDeProdutos.length > 0){
			...
			for(var i=0 ; i<listaDeProdutos.length; i++){
				tabela+= "<tr>" +
					...
					"<td>"+   
						"<a onclick=\"COLDIGO.produto.exibirEdicao('"+listaDeProdutos[i].id+"')\"><img src='../../imgs/edit.png' alt='Edita registro'></a>" +
		...
		$.ajax({
			...
			success: function(produto){
				...
				var modalEditaProduto = {
						...
						close: function(){
							//caso usuario feche a caixa de edição, nao deve acontecer nada
						}
